...
export default function QuranCourses() {
  ...
  const filteredCourses = courses.filter((course) => {
    const matchesGender =
      selectedGender === "all" || course.gender === selectedGender;
        ...
        >
          <div className="flex justify-center">
            <TabsList className="bg-white shadow-md p-1">
              {/* ✅ التعديل هنا: إضافة onClick={() => refetch()} للزر */}
              <TabsTrigger
                value="browse"
                className="px-8"
                onClick={() => refetch()}
              ...
              {user && (
                <TabsTrigger value="my_courses" className="px-8">
                  {t("my_courses")} ({myEnrollments.length})
                </TabsTrigger>
              )}
            </TabsList>
          </div>
                        ...
                        {enrollment.status === "approved" && (
                          <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-2 text-sm text-gray-600">
                              ...
                              <span>
                                {t("course_schedule")}: {course.schedule}
                              </span>
                            </div>
                          </div>
                        )}
